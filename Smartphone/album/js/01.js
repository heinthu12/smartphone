window.addEventListener
    ...
    var color = document.querySelectorAll('#color,button');
    var clear = document.querySelector('div.clear button');
    //色を変更する関数change color
    ...
    if(HTMLCanvasElement){
        ...
        targetColor = e.target.getAttribute('value');
    ...
    //3色の色をクリックしたときに関数changeColor を call
    for(var i = 0;i < color.length;i++){
        color.item(i).addEventListener(
        'click',changeColor,false);
    }
    //clear ボタンをクリックしたときにcanvasを初期化する
    clear.addEventListener(
        'click',
        function(e){
        //clearボタンのクリックときに実行するコード
        e.preventDefault();
